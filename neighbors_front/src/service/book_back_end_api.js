...
class BookBackEndAPI {
    //책 등록하기
    async addBook(addBooks,token) {
        const data = await axios.post("http://3.36.109.233:8080/hellobook/books", addBooks,token)
        return data;
    }
    ...
    //특정 도서 수정하기
    async changeBookDetail(editBook,token) {
        const data = await axios.post(`http://3.36.109.233:8080/hellobook/books/edit`,editBook, token);
        return data;
    }
}
